...
- The **$k$ standardized moment** of $X$ is $E\left(\left(\frac{X-\mu}{\sigma}\right)^k\right)$.
...
A common measure of tail behavior is the **excess kurtosis**. The excess kurtosis of a random variable $X$ is the shifted fourth standardized moment:
...
Note: Some authors write about the kurtosis instead, which does not have the subtract 3 in equation \@ref(eq:4-kurt), and so compare the kurtosis with 3, instead of comparing the excess kurtosis with 0. 
...
```{r 4-U, fig.cap = "PDF of U(a,b). Picture from [wikipedia](https://en.wikipedia.org/wiki/Continuous_uniform_distribution)", echo = FALSE}
...
```{r 4-Ucdf, fig.cap = "CDF of U(a,b). Picture from [wikipedia](https://en.wikipedia.org/wiki/Continuous_uniform_distribution)", echo = FALSE}
...
```{r 4-KDE, fig.cap = "Common Kernels. [Picture adapted from Computational Probability](https://tgstewart.cloud/compprob/kde.html)", echo = FALSE}
...
Equations \@ref(eq:4-EX), \@ref(eq:4-var), \@ref(eq:4-skew), and \@ref(eq:4-kurt) are used to obtain the mean, variance, skewness, and excess kurtosis of a known distribution from a random variable. To calculate these quantities based on a sample of observed data, $x_1, x_2, \cdots, x_n$, we use:
...
moments::kurtosis(Data$interest_rate) - 3 ##greater than 0
```

So our data is also right skewed and heavy tailed. Note that the `kurtosis()` function from the `moments` package reports the kurtosis, so we subtract 3 from its reported value to obtain the excess kurtosis. 
