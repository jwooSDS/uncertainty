...
A common inequality wthat is used for probability is **Chebyshev's inequality**. It provides an upper bound on the probability that a random variable is at least a certain distance from its mean. Let $X$ be a random variable with mean $\mu$ and variance $\sigma^2$. Then for any $a>0$, 
...
Using equation \@ref(eq:6_CIalt), we can say the following about the upper bond on the probability that a random variable is at least 1, 2, and 3 standard deviations from its mean:
...
The **Law of Large Numbers (LLN)** states that as $n$ gets larger, the sample mean $\bar{X}_n$ converges to the true mean $\mu$. In other words,the sample mean tends to get closer to the population mean with larger sample sizes. The key word here is tends to, it is not a guarantee that the sample mean always gets closer to the population mean whenever $n$ gets larger, but it generally does.

Some implications of the LLN:

- Sample means from larger samples (assuming the sample was obtained randomly) tend to be closer to the true mean than sample means from smaller samples. This is why we tend to trust results based on a larger sample size.

- We can use simulations to verify theoretical results, since these results usually require us to simulate data based on a large number of independent replications. 
...
1. The code below gives a visual representation of the LLN. The code does the following:
...
```{r 4-LLN1, fig.cap="LLN Example 1"}
n<-500 ##make this big, but not too big otherwise picture is difficult to see

X<-array(NA,n) ##initialize an array to store the value of each simulated data point
xbars<-array(0,n) ##initialize an array to store the value of the sample mean after each simulated data point
...

{
  ...
  X[i]<-rnorm(1) ##simulate one data point from standard normal and store it
  xbars[i]<-mean(X, na.rm=TRUE) ##find the sample mean after each simulated data point is generated

}

##create visual. LLN says that as n gets larger, the value of the sample mean tends to get closer to 0.
##remember that the true mean is 0 since each data point was simulated from standard normal
plot(xbars, type="l", main="Xbar vs Sample Size", ylab="Sample Mean", xlab="n")
abline(h=0, col="blue") ##overlay true mean for easy comparison
```
