...
This module is based on [OpenIntro Statistics](https://www.openintro.org/book/os/) (Diez, Ceytinka-Rundel, Barr), Chapter 2.1. Please note that I cover additional topics, and skip certain topics from the book. 
...
*Note:* do not get too torn up about the details in the code to produce this dot plot. I have chosen to present the dot plot this way to highlight how we use it, without getting bogged down in the details of how it can be produced. We will not be using dot plots in this class. 
...
```{r hist, fig.cap="Histogram for 50 Interest Rates"}
...
```{r hist05, fig.cap="Histogram for 50 Interest Rates, with Bin Width 0.5"}
...
```{r}
sort(Data$interest_rate)[13] 
sort(Data$interest_rate)[38]
```
...
*Thought question*: What does it mean if the sample variance of a set of observations is 0? Why does this indicate that there is little (or no) uncertainty about the set of observations?

Another related measure is the sample **standard deviation**, which is the square root of the sample variance. Similar to the variance, larger values indicate more spread in the data. 
