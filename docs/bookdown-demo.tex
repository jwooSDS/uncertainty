...
\item
  Compare the metric with some sort of threshold to assess if our observed data deviates ``far enough'' or not, and then make a decision.
\end{enumerate}
...
\label{eq:8-teststatMean}
\end{equation}

where \(SE(\bar{x}_n)\) is \(\frac{s}{\sqrt{n}}\). This can be called a \textbf{\(t\) statistic} to reflect that our test statistic is based on the \(t\) distribution.
...
Using equation \eqref{eq:8-teststatMean}, the \(t\) statistic is:
...
\subsubsection{Writing Conclusions}\label{writing-conclusions}

\subsubsection{Type I and Type II Errors}\label{type-i-and-type-ii-errors}
