...
\chapter*{Preface (Summer 2026 update)}\label{preface-summer-2026-update}
\addcontentsline{toc}{chapter}{Preface (Summer 2026 update)}
...
\subsection{Indepedence of Discrete RVs}\label{indepedence-of-discrete-rvs}
...
Now suppose the winnings is now defined as the square of the number of dots the die shows. Let \(T\) denote the new winnings, so \(T = X^2\). Since this is a non linear transformation, \(E(T) = E(X^2)\) may not equal to \(E(X)^2\):
