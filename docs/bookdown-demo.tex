...
Some estimators have known distributions, for example the sample mean, \(\bar{X}\). Using the CLT from Section \ref{CLT}, we know that as long as \(X_1, \cdots, X_n\) are i.i.d. from any distribution with finite mean \(\mu\) and finite variance \(\sigma^2\), and it \(n\) is large enough, then \(\bar{X}\) is approximately \(N(\mu, \frac{\sigma^2}{n})\). Looking at the plots in Figure \ref{fig:7-bias} and Figure \ref{fig:7-consistent}, we see that the density plots for \(\bar{X}\) are bell-shaped, which verifies the CLT.
