...
The ML estimators are just one kind of estimators. These are found by maximizing the likelihood function. We can find estiamtors in other ways (method of moments, or any other method). An estimator is any function that uses the data and calculates a number from the data. As estimator is \(\hat{\Theta}(\boldsymbol{X})\). We call \(\hat{\Theta}\) an estimator of \(\theta\).
