...
This module is based on Introduction to Probability for Data Science (Chan), Chapter 8.1 and 8.2. You can access the book for free at https://probability4datascience.com. Please note that I cover additional topics.
...
```{r 7-MOM, fig.cap="Density Plot of Birth Weights. Normal Curve with Parameters Estimated by MOM Overlaid", message=FALSE}
library(MASS)
...
- The first moment is $E(X) = \mu$, since $X \sim N(\mu, \sigma^2)$.
- The first sample moment is $M_1 = \frac{1}{n} \sum_{i=1}^n x_i = \bar{xX}$. 
...
- We could choose to maximize the log-likelihood function instead. It is often easier to work with the log-likelihood function. The solution will be the same. 

We re-visit Examples 1 and 2 from the previous subsection.

#### Example 1: Bernoulli

Let $X_1, \cdots, X_n$ be i.i.d. $Bern(p)$. Find the ML estimate for $p$. We will work with the log-likelihood function, which we had found to be 
...
We can easily maximize any function by taking its first derivative and setting it to 0. So we take the first derivative of $\ell (p | \boldsymbol{x})$:
...
Let $X_1, \cdots, X_n$ be i.i.d. $N(\mu, \sigma^2)$. Find the ML estimates for $\mu$ and $\sigma^2$. Again, we will work with the log-likelihood function, which we had found to be 

$$
\ell (\mu, \sigma^2 | \boldsymbol{x}) = -\frac{n}{2} \log (2 \pi \sigma^2) - \frac{1}{2 \sigma^2} \sum_{i=1}^n (x_i - \mu)^2.
$$

Notice we have two parameters $\mu$ and $\sigma^2$, so we have to take partial derivatives for both:
...
We have learned about a couple of different methods to estimate parameters. While these two methods are common, there are not the only methods to estimate parameters. One question will be evaluating how "good" are our estimates? Before going into this evaluation, we define a few concepts that are used in this evaluation. 
...
The ML estimators are just one kind of estimators. These are found by maximizing the likelihood function. We can find estimators in other ways (method of moments, or any other method). An estimator is any function that uses the data and calculates a number from the data. As estimator is $\hat{\Theta}(\boldsymbol{X})$. We call $\hat{\Theta}$ an estimator of $\theta$.
...
The results from the MC simulation matches with theory.  $\hat{\Theta}_1(\boldsymbol{X}) = \frac{\sum_{i=1}^n X_i}{n}$ is an unbiased estimator for $\mu$, whereas $\hat{\Theta}_2(\boldsymbol{X}) = X_1 + 2$ is a biased estimator.
...
We can go back to the example from the previous subsection. Consider $X_1, \cdots, X_n$ i.i.d. Normal with unknown mean $\mu$ and variance $\sigma^2$ that is known. we can define two estimators of $\mu$ as $\hat{\Theta}_1(\boldsymbol{X}) = \frac{\sum_{i=1}^n X_i}{n}$ and $\hat{\Theta}_2(\boldsymbol{X}) = X_1 + 2$. Derive the variance of both estimators.
...
Some estimators have known distributions, for example the sample mean, $\bar{X}$. From Section \@ref(CLT), we know that as long as $X_1, \cdots, X_n$ are i.i.d. from any distribution with finite mean $\mu$ and finite variance $\sigma^2$, and it $n$ is large enough, then $\bar{X}$ is approximately $N(\mu, \frac{\sigma^2}{n})$. Looking at the plots in Figure \@ref(fig:7-bias) and Figure \@ref(fig:7-consistent), we see that the density plots for $\bar{X}$ are bell-shaped, which verifies the CLT.

Some other common estimators also have known distributions, such as the sample proportion as an estimator for the population proportion, the sample variance as an estimator for the population variance, as well as ML estimators for linear regression and logistic regression models. Feel free to look these up on the internet. 

If the sampling distribution of an estimator is known and follows well known distributions, we can easily perform probability calculations involving estimators, for example, how likely are we to observe a sample mean that is more than 2 standard errors away from its true mean? We will see how to carry these calculations in the next couple of modules.

However, other estimators may not have known distributions, for example, the sample median as an estimator for the population median has no known sampling distribution. We are unable to calculate probabilities associated with such estimators? It turns out there exist methods called resampling methods that allow us to approximate these calculations. We will cover the bootstrap, which is a commonly used resampling method, in a future module. 
...
There exist **nonparametric methods** in estimation. We have actually seen one such method (without calling it nonparametric), which is kernel density estimation (KDE) in Section \@(KDE). KDE is used to estimate the PDF of a random variable so that we can visualize its distribution. If you look back at KDE, you will notice we made no assumption abou the distribution of the random variable. This is one of the fundamental differences between parametric and nonparametric estimation: the former assumes a distribution for the data, the latter does not. 
