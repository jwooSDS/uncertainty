...
<iframe width="560px" height="320px" allowfullscreen="true" allow="autoplay *" title="Module 10: Distribution of Y given X in Regression" src="https://virginiauniversity.instructuremedia.com/embed/445744ac-5422-496a-943e-6b2912130c4a" frameborder="0"></iframe>
...
The notation on the left hand side of equation \@ref(eq:10-SLR) denotes the **expected value** of the response variable, for a fixed value of the predictor variable. Therefore, the regression coefficients can be interpreted in the following manner:
...
<iframe width="560px" height="320px" allowfullscreen="true" allow="autoplay *" title="Module 10: Assessing Assumptions in Linear Regression" src="https://virginiauniversity.instructuremedia.com/embed/6cc805a1-4274-4693-91de-c359d90e4e44" frameborder="0"></iframe>
...
respectively. Equation \@ref(eq:10-fitted) is called the **fitted line**. The line that was overlaid in the scatter plot for the toy example in Figure \@ref(fig:10-scatter) represents the fitted line. Equation \@ref(eq:10-fitted-model) is called the **estimated SLR model**.
...
In equation \@ref(eq:10-fitted-model), notice we use $e$ to denote the **residual**, or in other words, the "error" in the sample. 

From equations \@ref(eq:10-fitted) and \@ref(eq:10-fitted-model), we have the following quantities that we can compute:
...
```{r 10-res, fig.cap = "Example of Residuals. Picture from [Statology](https://www.statology.org/residuals/)", echo = FALSE}
...
We compute the estimated coefficients $\hat{\beta}_1,\hat{\beta}_0$ using the **method of least squares**, i.e. choose the numerical values of $\hat{\beta}_1,\hat{\beta}_0$ that minimize $SS_{res}$ as given in equation \@ref(eq:10-SSres). We find the line which minimizes the sum of all the squared residuals on the scatter plot, hence the name method of least squares.
...
<iframe width="560px" height="320px" allowfullscreen="true" allow="autoplay *" title="Module 10: Method of Least Squares" src="https://virginiauniversity.instructuremedia.com/embed/129b20c0-b2fa-4bfb-a8e5-e810840cfb95" frameborder="0"></iframe>
...
We then take the partial derivatives of equation \@ref(eq:10-SLRloglike) with respect to $\hat{\beta}_1$ and $\hat{\beta}_0$, set these two partial derivatives equal to 0, and solve these two equations for $\hat{\beta}_1$ and $\hat{\beta}_0$. We end up with the same solutions as the method of least squares.

The code below finds the estimated coeffficients for our study time example. The `glm()` function fits via method of maximum likelihood, whereas the `lm()` function from earlier fits via the method of least squares.
...
Note that in equations \@ref(eq:10-varb1) and \@ref(eq:10-varb0), we use $s^2 = \frac{SS_{res}}{n-2} = MS_{res}$ to estimate $\sigma^2$, the variance of the errors, since $\sigma^2$ is a unknown value.
...
- $\hat{\beta}_1 =$ `r summary(result)$coefficients[2,1]`. The estimated slope informs us the predicted gift aid decreases by `r -summary(result)$coefficients[2,1]` thousands of dollars (or \$43.07) per unit increase in family income.
...
<iframe width="560px" height="320px" allowfullscreen="true" allow="autoplay *" title="Module 10: Linear Regression in R" src="https://virginiauniversity.instructuremedia.com/embed/e6ab985e-7b0f-4fe3-8eaa-90f8611b7f70" frameborder="0"></iframe>
...
The **mean-squared error (MSE)** is a commonly used metric to assess model accuracy. The MSE of predictions is
